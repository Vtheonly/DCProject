"""
DSP Runner Agent - FIXED VERSION

This file contains the critical bug fixes for the C++ DSP integration.

ISSUES FIXED:
1. result.trip should be result.trip.triggered (line ~54, ~71)
2. d1_peak was hardcoded to 0.0 instead of using result.d1_peak (line ~69)

Copy this file to: src/agents/processing/dsp_runner.py
"""
...
class DSPRunnerAgent(BaseAgent):
    ...
    def on_sample(self, event: VoltageSampleEvent):
        ...
        try:
            ...
            # 1. Fast Trip Logic
            # FIX: Use result.trip.triggered instead of result.trip
            # The C++ TripResult object has a 'triggered' boolean attribute
            ...
            if result.window_ready:
                ...
                # FIX: Use actual d1_peak from C++ result (was hardcoded to 0.0)
                # FIX: Use result.trip.triggered for is_faulty (was using result.trip object)
